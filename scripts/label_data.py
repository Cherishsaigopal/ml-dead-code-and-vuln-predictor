...
def label_dataset(input_csv, output_csv):
    ...
    # -------------------------
    df["label_deadcode"] = (
        (df["unreachable_blocks"] > 0) |
        (df["unreachable_ratio"] > 0)
    ).astype(int)
    ...
    juliet = file_col.str.contains("juliet-test-suite-c")

    df.loc[juliet & func_col.str.contains("bad"), "label_vuln"] = 1
    df.loc[juliet & func_col.str.contains("good"), "label_vuln"] = 0
